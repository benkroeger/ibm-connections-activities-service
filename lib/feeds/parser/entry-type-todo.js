...
function parseActivityEntry(entryNode) {
  ...
  // parse in-reply-to
  const inReplyToSelector = 'thr:in-reply-to';
  const inReplyToNode = parserUtils.selectXPath(inReplyToSelector, entryNode, true);
  if (inReplyToNode) {
    parsedResult.inReplyTo = {
      ref: inReplyToNode.getAttribute('ref'),
      type: inReplyToNode.getAttribute('type'),
      href: inReplyToNode.getAttribute('href'),
      source: inReplyToNode.getAttribute('source'),
    };
  }
