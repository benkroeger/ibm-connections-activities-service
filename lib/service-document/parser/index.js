...
const collectionSelectors = Object.keys(collectionNameToId).reduce((result, current) => {
  const idSelector = './atom:id/text()';
  const subStrLen = collectionNameToId[current].length - 1;
  /* eslint-disable no-param-reassign */
  result[current] = [
    '/app:service/app:workspace/app:collection',
    `[substring(${idSelector}, string-length(${idSelector}) - ${subStrLen}) = "${collectionNameToId[current]}"]`,
  ].join('');
